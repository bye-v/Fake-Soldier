...
public class Stage04Director : StageDirector
{
    ...
    {
        yield return ShowStageTitle("1980년 5월 21일\n전남도청");

        ShakeCamera(0.5f, 0.12f);
        ...
        yield return PlayDialogue(new[]
        {
            ...
            new DialogueLine { speaker = "시민군",  text = "쏘지 마시오! 우리는 시민이오! 우리 가족이 저 골목에 있소!" },
            new DialogueLine { speaker = "나 (속)", text = "저 목소리에서 어제 본 여성의 울음이 겹친다. 이틀 전 아이의 눈빛도." },
            new DialogueLine { speaker = "나 (속)", text = "총이 무거워졌다. 방아쇠에 걸린 손가락이 굳어간다. 10초..." },
        });

        ShakeCamera(0.6f, 0.18f);
